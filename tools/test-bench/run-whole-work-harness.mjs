...
//
// דילוג על בנייה מחדש: WORDAI_WHOLE_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-whole-harness');
const BUNDLE = path.join(SCRATCH, 'out-whole', 'sf.mjs');
...
if (process.env.WORDAI_WHOLE_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'whole', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
