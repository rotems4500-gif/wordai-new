...
//
// דילוג על בנייה מחדש: WORDAI_REVIEW_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-review-unit');
const BUNDLE = path.join(SCRATCH, 'out-review', 'sf.mjs');
...
if (process.env.WORDAI_REVIEW_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'review', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
