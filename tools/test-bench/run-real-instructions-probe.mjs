...
//
// דילוג על בנייה מחדש: WORDAI_REALINSTR_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-realinstr');
const BUNDLE = path.join(SCRATCH, 'out-realinstr', 'sf.mjs');
...
if (process.env.WORDAI_REALINSTR_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'realinstr', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
