...
//
// דילוג על בנייה מחדש: WORDAI_PREVIEW_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-preview');
const BUNDLE = path.join(SCRATCH, 'out-preview', 'sf.mjs');
...
if (process.env.WORDAI_PREVIEW_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'preview', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
