...
//
// דילוג על בנייה מחדש: WORDAI_E2E_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-e2e');
const BUNDLE = path.join(SCRATCH, 'out-e2e', 'sf.mjs');
...
if (process.env.WORDAI_E2E_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'e2e', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
