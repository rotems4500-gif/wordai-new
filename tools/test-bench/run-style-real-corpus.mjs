// run-style-real-corpus.mjs — מפענח את ה-config של האפליקציה (DPAPI), בונה את
// harness הסגנון על הקורפוס האמיתי ומריץ אותו. אותה שיטה של run-openers-real-corpus.
//
//   node tools/test-bench/run-style-real-corpus.mjs
//
// דילוג על בנייה מחדש: WORDAI_STYLEPROBE_REBUILD=0
// קורפוס אחר:        WORDAI_STYLE_CORPUS=<dir>
// יעד ה-dump:        WORDAI_STYLE_DUMP=<file>
//
// ⚠️ ה-bundle חייב לשבת בתוך הפרויקט: הוא external ל-mammoth, ורזולוציית ESM
// מטפסת מהקובץ עצמו — מחוץ לפרויקט אין node_modules והייבוא נופל.
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH || path.join(DIR, '.styleprobe-scratch');
const BUNDLE = path.join(SCRATCH, 'out-styleprobe', 'sf.mjs');
...
console.log(cfg ? 'provider config loaded ✓' : 'no provider config — LLM steps will report no-provider');

if (process.env.WORDAI_STYLEPROBE_REBUILD !== '0') {
  console.log('building styleprobe bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'styleprobe', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
