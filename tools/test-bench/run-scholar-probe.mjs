// run-scholar-probe.mjs — מפענח config (DPAPI), בונה ומריץ את בדיקת ה-Scholar החיה.
//   node tools/test-bench/run-scholar-probe.mjs
// דילוג על בנייה: WORDAI_SCHOLAR_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-scholar-probe');
const BUNDLE = path.join(SCRATCH, 'out-scholar', 'sf.mjs');
...
if (process.env.WORDAI_SCHOLAR_REBUILD !== '0') {
  console.log('building probe bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'scholar', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
