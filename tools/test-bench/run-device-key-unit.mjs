...
//
// דילוג על בנייה מחדש: WORDAI_DEVKEY_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-devkey');
const BUNDLE = path.join(SCRATCH, 'out-devkey', 'sf.mjs');
...
if (process.env.WORDAI_DEVKEY_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'devkey', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
