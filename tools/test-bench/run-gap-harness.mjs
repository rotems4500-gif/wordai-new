// run-gap-harness.mjs — מפענח את ה-config של האפליקציה (DPAPI), בונה את
// ה-harness ומריץ אותו. אותה שיטה של server.mjs, בלי להרים שרת.
//
//   node tools/test-bench/run-gap-harness.mjs
//
// דילוג על בנייה מחדש: WORDAI_GAP_REBUILD=0
...
const SCRATCH = process.env.WORDAI_VERIFY_SCRATCH
  || path.join(process.env.LOCALAPPDATA || '', 'Temp', 'wordai-gap-harness');
const BUNDLE = path.join(SCRATCH, 'out-gap', 'sf.mjs');
...
if (process.env.WORDAI_GAP_REBUILD !== '0') {
  console.log('building harness bundle…');
  await run('npx', ['vite', 'build', '--config', 'vite.verify.config.mjs'], {
    cwd: PROJECT,
    env: { ...process.env, WORDAI_VERIFY_ENTRY: 'gap', WORDAI_VERIFY_SCRATCH: SCRATCH },
  });
}
