...
// עם דגימות AI אמיתיות מ-Gemini Flash, בשלושה מצבים: "normal" (AI רגיל), "stealth"
// (מודל שמקבל רשימת מניעה מפורשת של המרקרים הידועים שלנו — מדמה AI "מבוית"), ו-"casual"
// (טון קליל-אישי — ר' CASUAL_GENRES; פספסנו טקסט בלוג-טיולים אמיתי בגלל היעדר רגיסטר כזה).
// שתי צורות: בלוקים קצרים (120-200 מילה) וצורות "מסמך שלם" — form=full-work (עבודות
// אקדמיות אמיתיות, 1,500-2,500 מילה) ו-form=full-doc (ז'אנרים קליליים, 400-800 מילה,
// עם כותרות + פסקאות מרובות מופרדות בשורה ריקה — נגד הממצא ש-88% מבלוקי ה-AI הם
// פסקה בודדת, כלומר כללי מבנה-מסמך לא ניתנים לאימות עליהם).
//
...
//   כל בלוק קודמת לו שורת כותרת `#genre=<slug> mode=<normal|stealth|casual>[ form=<full-work|full-doc>]`,
//   ומופרד מהבא אחריו בשורת `===`.
//
// שימוש:
//   node tools/detector-train/generate-ai-corpus.mjs            # ~90 בלוקים + עבודות/מסמכים מלאים
//   node tools/detector-train/generate-ai-corpus.mjs --double    # שתי סבבים, נושאים שונים, ~180 (רק ז'אנרים המקוריים)
...
// ── ז'אנרים "רגיסטר קליל" — הרחבת קורפוס בעקבות ממצא מחקר (אוגוסט 2026):
// 88% מבלוקי ה-AI הם פסקה בודדת (כללים ברמת-מסמך לא ניתנים לאימות), ורגיסטרים
// קליליים חסרים לגמרי — טקסט בלוג-טיולים אמיתי שנוצר ע"י AI פוספס לגמרי ע"י הגלאי.
// 8 ז'אנרים חדשים × 10 נושאים כל אחד, בנוסף למצב "casual" (ר' buildPrompt) ולצורת
// full-doc (מסמך שלם עם כותרות + פסקאות מופרדות בשורה ריקה — ר' buildFullDocPrompt).
const CASUAL_GENRES = [
  {
    slug: 'travel-blog', label: 'בלוג טיולים בארץ',
    topics: [
      'סוף שבוע בצפת ובכפרי האמנים שסביבה',
      'טיול משפחות בשמורת עין גדי',
      'חוף הבונים וסביבתו — יום ים מושלם',
      'מסלול טיול קצר בהרי ירושלים',
      'סיור רגלי בשוק מחנה יהודה',
      'לילה אחד באוהלים בכרמל',
      'טיול אופניים סביב הכנרת',
      'יום בנחל דוד ובשמורת נחל פרת',
      'סיור טעימות ביקבי רמת הגולן',
      'חופשה קצרה באילת ובאיזור המפרץ',
    ],
  },
  {
    slug: 'how-to-guide', label: 'מדריך מעשי "X דברים שכדאי לדעת"',
    topics: [
      'שבעה דברים שכדאי לדעת לפני שכירת דירה ראשונה',
      'חמישה דברים שכדאי לדעת לפני קניית רכב יד שנייה',
      'שישה דברים שכדאי לדעת לפני טיסה ראשונה עם תינוק',
      'עשרה דברים שכדאי לדעת לפני התחלת דיאטה',
      'ארבעה דברים שכדאי לדעת לפני פתיחת עסק עצמאי',
      'שמונה דברים שכדאי לדעת לפני רכישת ביטוח בריאות פרטי',
      'חמישה דברים שכדאי לדעת לפני מעבר דירה',
      'שישה דברים שכדאי לדעת לפני התחלת לימודים אקדמיים',
      'שבעה דברים שכדאי לדעת לפני אימוץ כלב',
      'ארבעה דברים שכדאי לדעת לפני השקעה בשוק ההון',
    ],
  },
  {
    slug: 'product-review', label: 'ביקורת מוצר/שירות',
    topics: [
      'ביקורת על שואב אבק רובוטי לבית',
      'חוויית שימוש עם אוזניות אלחוטיות חדשות',
      'ביקורת על שירות משלוחי מזון מהיר',
      'התרשמות ממכונת קפה ביתית חדשה',
      'ביקורת על אפליקציית ניהול תקציב אישי',
      'חוויה עם חברת השכרת רכב',
      'ביקורת על מזרן אורתופדי חדש',
      'התרשמות משירות סטרימינג חדש',
      'ביקורת על מותג בגדי ספורט',
      'חוויית שימוש עם שירות ניקיון לבית',
    ],
  },
  {
    slug: 'forum-answer', label: 'תשובה מפורטת בפורום',
    topics: [
      'תשובה בפורום על איך להתמודד עם חרדת בחינות',
      'תשובה בפורום על בחירת מסלול לימודים אקדמי',
      'תשובה בפורום על התמודדות עם שכן רועש',
      'תשובה בפורום על טיפים לחיסכון בהוצאות חשמל',
      'תשובה בפורום על איך להתחיל להתאמן בגיל מבוגר',
      'תשובה בפורום על התמודדות עם דחיינות בעבודה',
      'תשובה בפורום על בחירת מעון לילד',
      'תשובה בפורום על התמודדות עם לחץ בעבודה חדשה',
      'תשובה בפורום על טיפים למעבר לעבודה מהבית',
      'תשובה בפורום על התמודדות עם קונפליקט עם שותף לדירה',
    ],
  },
  {
    slug: 'social-post', label: 'פוסט לינקדאין/פייסבוק מקצועי-אישי',
    topics: [
      'פוסט לינקדאין על סיום פרויקט משמעותי בעבודה',
      'פוסט על מעבר תפקיד ולקחים מהדרך',
      'פוסט על השתתפות בכנס מקצועי',
      'פוסט אישי-מקצועי על איזון בין עבודה לחיים',
      'פוסט על הצלחה של הצוות ברבעון האחרון',
      'פוסט על התחלת תפקיד ניהולי חדש',
      'פוסט על לקח מכישלון מקצועי',
      'פוסט על חשיבות הלמידה המתמדת בעבודה',
      'פוסט על שיתוף פעולה עם קולגות מעניין',
      'פוסט על חגיגת שנתיים לעבודה עצמאית',
    ],
  },
  {
    slug: 'parenting-health', label: 'תוכן הורות/בריאות פופולרי',
    topics: [
      'טיפים להתמודדות עם התקפי זעם אצל פעוטות',
      'איך לעודד הרגלי שינה בריאים אצל ילדים',
      'מדריך פופולרי לתזונה נכונה למתבגרים',
      'איך להתמודד עם חרדת פרידה בגן',
      'טיפים להורים לניהול זמן מסך אצל ילדים',
      'מדריך פופולרי להתמודדות עם התקררות אצל ילדים',
      'איך לבנות שגרת בוקר רגועה עם ילדים',
      'טיפים להתמודדות עם ריבים בין אחים',
      'מדריך פופולרי לשינה טובה יותר למבוגרים',
      'איך לעודד פעילות גופנית אצל ילדים בעידן המסכים',
    ],
  },
  {
    slug: 'recipe-instructions', label: 'מתכון והוראות',
    topics: [
      'מתכון לעוגת שוקולד קלה להכנה',
      'מתכון למרק עדשים ביתי',
      'מתכון לסלט קינואה קיצי',
      'מתכון לחומוס ביתי חלק',
      'מתכון לפשטידת ירקות פשוטה',
      'מתכון לעוף בתנור עם תפוחי אדמה',
      'מתכון ללחם בית ללא לישה',
      'מתכון לשקשוקה קלאסית',
      'מתכון לעוגיות שיבולת שועל',
      'מתכון לפסטה ברוטב עגבניות ביתי',
    ],
  },
  {
    slug: 'news-summary', label: 'סיכום חדשותי פופולרי',
    topics: [
      'סיכום חדשותי על עליית מחירי הדיור האחרונה',
      'סיכום חדשותי על שינויים ברפורמת התחבורה הציבורית',
      'סיכום חדשותי על התפתחויות בשוק ההייטק המקומי',
      'סיכום חדשותי על גל החום ומזג האוויר הקיצוני',
      'סיכום חדשותי על רפורמה במערכת הבריאות',
      'סיכום חדשותי על מגמות בשוק העבודה',
      'סיכום חדשותי על שביתת מורים במגזר החינוך',
      'סיכום חדשותי על עדכוני רגולציה בענף הרכב',
      'סיכום חדשותי על תקציב המדינה החדש',
      'סיכום חדשותי על מגמות תיירות בעונת הקיץ',
    ],
  },
];
...
- אל תפתח שני בלוקים שונים באותה מילה או באותה צורת פתיחה.`;
  ...
  // מצב "casual" — רגיסטר קליל, בדיוק מה שפספס את בלוג-הטיולים (ר' הערה ליד CASUAL_GENRES).
  if (mode === 'casual') {
    prompt += `

חשוב מאוד לגבי הטון: כתוב בגובה העיניים, טון קליל ואישי, בלי שפה פורמלית. כאילו אתה מספר לחבר,
לא כותב מסמך רשמי או עבודה אקדמית. מותר לשון דיבורית, גוף ראשון, וסלנג יומיומי מתון.`;
  }

  return prompt;
}

// ── צורת "מסמך שלם" (form=full-doc) — נגד ממצא המחקר ש-88% מבלוקי ה-AI הם פסקה
// בודדת: כללים ברמת-מבנה-מסמך (כותרות, מעברי פסקה, תבליטים) לא ניתנים לאימות על
// פסקה יחידה. כאן המודל מקבל הנחיה מפורשת לכותרות + פסקאות מרובות מופרדות בשורה ריקה.
function buildFullDocPrompt(genre, topic, mode) {
  let prompt = `כתוב מסמך שלם ומובנה בעברית תקנית בסגנון "${genre.label}", באורך של 400 עד 800 מילים, בנושא:
${topic}

חשוב מאוד לגבי המבנה של המסמך:
- כותרת ראשית קצרה בתחילת המסמך, ולפחות 2-3 כותרות משנה שמחלקות אותו לחלקים.
- הפרד בין כל שתי פסקאות בשורה ריקה אחת (פסקה, שורה ריקה, פסקה הבאה) — אסור פסקה רציפה ארוכה אחת.
- אם מתאים לסוג התוכן, שלב באחד מחלקי המסמך רשימת תבליטים בפורמט "כותרת קצרה: הסבר" (לא רק תבליט בודד).
- סיים בפסקת סיכום קצרה עם המלצה או מסקנה ברורה לקורא.
- אל תוסיף הערות מטא על עצמך או על תהליך הכתיבה — רק את גוף המסמך, כולל הכותרות והתבליטים.`;

  if (mode === 'casual') {
    prompt += `

טון הכתיבה: כתוב בגובה העיניים, טון קליל ואישי, בלי שפה פורמלית — כאילו אתה כותב לקוראים שמכירים
אותך, לא מגיש מסמך רשמי. מותר לשון דיבורית וגוף ראשון.`;
  }
...
async function main() {
  ...
  // ── ז'אנרים "רגיסטר קליל" — בלוקים קצרים, 3 מצבים (normal/stealth/casual) ──
  // 10 נושאים לז'אנר, 5 בלוקים לקריאה: מסובבים איזה 5 מתוך ה-10 נכנסים לכל מצב
  // (start = (mi*5) % 10) כדי לכסות את כולם בלי להכפיל את מספר הקריאות.
  const casualModes = ['normal', 'stealth', 'casual'];
  console.log(`\nז'אנרים קליליים חדשים: ${CASUAL_GENRES.length} × ${casualModes.length} מצבים = ${CASUAL_GENRES.length * casualModes.length} קריאות (בלוקים קצרים)`);
  for (const genre of CASUAL_GENRES) {
    for (let mi = 0; mi < casualModes.length; mi++) {
      const mode = casualModes[mi];
      const start = (mi * 5) % genre.topics.length;
      const topics = genre.topics.slice(start, start + 5);
      console.log(`→ ${genre.label} [${mode}]…`);
      const prompt = buildPrompt(genre, topics, mode);
      let raw;
      try {
        raw = await callGemini(key, prompt);
  ...
  // ── ז'אנרים קליליים — form=full-doc: מסמך שלם 400-800 מילים עם כותרות + פסקאות
  // מרובות (ר' הערה ליד buildFullDocPrompt). 2 מצבים בלבד (normal/casual) — stealth
  // לא כלול כאן כדי לא לנפח את התקציב; ה-AVOID_LIST כבר מיוצג בקצרים+בעבודות המלאות.
  const fullDocModes = ['normal', 'casual'];
  console.log(`\nז'אנרים קליליים — מסמך שלם: ${CASUAL_GENRES.length} × ${fullDocModes.length} מצבים = ${CASUAL_GENRES.length * fullDocModes.length} קריאות`);
  const countsByFormCasual = {};
  for (const genre of CASUAL_GENRES) {
    for (let mi = 0; mi < fullDocModes.length; mi++) {
      const mode = fullDocModes[mi];
      const topic = genre.topics[mi % genre.topics.length];
      console.log(`→ [מסמך שלם] ${genre.label} [${mode}]: ${topic}…`);
      const prompt = buildFullDocPrompt(genre, topic, mode);
      let raw;
      try {
        raw = await callGemini(key, prompt, { maxOutputTokens: 4096 });
      } catch (e) {
        console.warn(`  נכשל: ${e.message}`);
        await sleep(2000);
        continue;
      ...
      if (seen.has(fp)) {
        ...
        seen.add(fp);
        await appendBlocks([{ genre: genre.slug, mode, text, form: 'full-doc' }]);
        totalNew += 1;
        const ck = `${genre.slug} ${mode} (full-doc)`;
        countsByGenreMode[ck] = (countsByGenreMode[ck] || 0) + 1;
        countsByFormCasual[mode] = (countsByFormCasual[mode] || 0) + 1;
        console.log(`  ${wc} מילים → נוסף כבלוק אחד (יעד: 400-800)`);
      }
      await sleep(2000);
    }
  }
  ...
  console.log('\n=== סיכום ===');
  console.log(`בלוקים/עבודות/מסמכים חדשים שנוספו: ${totalNew}`);
  ...
  console.log(`מסמכים קליליים שלמים שנוספו: normal=${countsByFormCasual.normal || 0}, casual=${countsByFormCasual.casual || 0}`);
  console.log('הערה: עבודות/מסמכים מלאים נשמרים כבלוק === יחיד (לא מפוצלים). detector-eval מחלונן מסמכים אנושיים אבל מניקד בלוקי AI כמו שהם — מקובל לשלב הזה.');
