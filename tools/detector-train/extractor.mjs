...
//
// ⚠️ מאותה סיבה בדיוק, גם FORMAL_CONNECTORS / CLICHE_PHRASES / AI_REGISTER_PATTERN
// כבר אינם משוכפלים כאן: הם מיובאים מ-src/services/styleMarkers.shared.js (LEAF,
// מחרוזות בלבד), אותו קובץ שממנו מייבא styleAuthenticityService. sync-check.mjs
// אוכף שאף אחד מהשניים לא יחזיר לעצמו עותק מקומי.
...
import {
  FORMAL_CONNECTORS, CLICHE_PHRASES, AI_REGISTER_PATTERN,
} from '../../src/services/styleMarkers.shared.js';

// re-export — train.mjs / generate-ai-corpus.mjs מייבאים אותם מכאן מאז ומתמיד.
export { FORMAL_CONNECTORS, CLICHE_PHRASES };
...
// נבנה מאותה מחרוזת משותפת בדיוק (AI_REGISTER_PATTERN), כמו בגלאי עצמו.
export const AI_REGISTER_RE = new RegExp(AI_REGISTER_PATTERN, 'g');
