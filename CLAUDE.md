...
- **למידה ממשובי מרצים** — [lecturerProfileStore.js](src/services/lecturerProfileStore.js) (פרופיל פר-מרצה + לקחים גלובליים, IndexedDB, מסונכרן ענן בסכמה v5) · [docxFeedbackExtract.js](src/services/docxFeedbackExtract.js) (הערות Word + track-changes מ-OOXML, JSZip+DOMParser) · [feedbackDiffService.js](src/services/feedbackDiffService.js) (diff הגשה↔מוחזר) · [lecturerRulesService.js](src/services/lecturerRulesService.js) (זיקוק אירועים→לקחים; שער אנטי-הזיה: לקח בלי ציטוט event נזרק) · `extractPdfAnnotations` ב-materialExtractBrowser. UI: `LecturerProfilePanel` + `GradedReturnWizard` (מ-FileMenu→הגדרות→מה המנוע למד). הזרקה: `buildLecturerRulesBlock` ב-buildProjectContextBlock / buildScaffoldContextBlock / doc-gen / סוכן lecturer; בדיקות דטרמיניסטיות ב-reviewDraft (`lecturerRules` ב-opts). בדיקה: `WORDAI_VERIFY_ENTRY=lprof` → `lecturer-learning-unit.mjs`.
