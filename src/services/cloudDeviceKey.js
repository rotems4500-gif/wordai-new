...
// להקליד את סיסמת ההצפנה בכל פתיחה.
//
...
//   - דפדפן/PWA: CryptoKey ב-IndexedDB. שורד reload, מוגבל ל-origin הזה.
//
...
export async function saveDeviceKey(uid, dek) {
  ...
  try {
    ...
    await idbPut(uid, dek); // structured clone שומר CryptoKey כמות שהוא
    return true;
  } catch {
    return false;
  }
...
export async function loadDeviceKey(uid) {
  ...
  try {
    if (isDesktop()) {
      ...
      const b64 = data?.dek;
      if (!b64) return null;
      return await crypto.subtle.importKey('raw', b64ToBytes(b64), { name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
    }
...
// מוחק את ה-DEK השמור (שכח מכשיר / השבתת הצפנה).
