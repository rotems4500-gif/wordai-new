...
export function normalizeLecturerName(name) {
  ...
  // ⚠️ הגרשיים מוסרים **לפני** התארים: ד"ר (גרש ישר) ו-ד״ר (גרשיים עבריים) הם
  // אותו תואר, ובלי האיחוד הזה שני האיותים יצרו שני מרצים נפרדים עם אותו שם.
  s = s.replace(/["'״׳]/g, '');
  s = s.replace(/(?:^|\s)(?:דר|פרופ|פרופסור|מר|גב|עוד)(?=\s)/g, ' ');
