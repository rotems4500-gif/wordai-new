...
// ⚠️ הורחב אחרי מדידה על מטלת דיני תקשורת אמיתית. הפתיח שלה מצהיר על הדוקטרינה
// במפורש — "תיאור מקרה **העוסק בסוגיות של** חופש הביטוי, פרטיות, לשון הרע,
// אתיקה עיתונאית" — וניסוח זה לא היה ברשימה. התוצאה: כל שש השאלות אחזרו לפי
// שמות הדמויות במקרה (דליה/יקיר/דניאל) במקום לפי הדוקטרינה, ו-4 מהן חזרו ריקות.
// זהו הניסוח הרווח במטלות מבוססות-קייס, שהן נפוצות במשפטים ובמדעי החברה.
const DOCTRINE_ANCHOR_RE = /(?:העקרונות של|הגותו של|על פי|מספרו של|בסוגיות של|העוסק(?:ת)? ב|בנושא|בתחום)\s+([^.,;()[\]\n]{2,40}?)(?=[.,;()[\]\n]|\s+(?:אשר|לגבי|אודות|בנוגע|ביחס|ש)\s|$)/;

// עוגן ברמת-המסמך: רשימת הדוקטרינות שהמרצה מצהיר עליה בפתיח. במטלת קייס
// הכותרת של כל שאלה היא **עובדות המקרה**, והמסגרת האנליטית מוצהרת פעם אחת
// בראש המסמך. בלי זה כל שאלה מחפשת לפי שמות הדמויות.
const DOCTRINE_LIST_RE = /(?:בסוגיות של|העוסק(?:ת)? ב|בנושא|בתחום)\s+([^.;()[\]\n]{10,160})/;

/**
 * עוגן דוקטרינרי מפתיח המטלה — מחזיר את *רשימת* התחומים כמחרוזת אחת, לשימוש
 * כגשוש אחזור לסעיפים שהכותרת שלהם היא עובדות-מקרה.
 * @returns {string|null}
 */
export function extractDoctrineScope(text) {
  const src = String(text || '').replace(/\s+/g, ' ');
  const m = src.match(DOCTRINE_LIST_RE);
  if (!m) return null;
  const phrase = String(m[1] || '').trim().replace(/\s+ו?בהן.*$/, '');
  const words = countWords(phrase);
  return words >= 3 && words <= 25 ? phrase : null;
}
...
export function parseAssignmentSpec(text, { title = '' } = {}) {
  ...
  return {
    ...
    structure,
    sections,
    // המסגרת האנליטית המוצהרת בפתיח ("סוגיות של חופש הביטוי, פרטיות, לשון הרע").
    // ⚠️ חייב להיחלץ **כאן**, מהטקסט המלא: ה-spec מחזיק רק כותרות וגופי סעיפים,
    // והפתיח יושב לפניהם. evidenceMatchService משתמש בזה כגשוש לסעיפים שכותרתם
    // היא עובדות-מקרה — ר' ההסבר שם.
    doctrineScope: extractDoctrineScope(src),
    warnings,
    confidence,
    parsedAt: Date.now(),
  };
