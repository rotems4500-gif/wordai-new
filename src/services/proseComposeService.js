...
const SOURCE_VOICE_RE = /(?:^|\s)(?:ציינתי|הזכרתי|אמרתי|כתבתי|הצגתי|פירטתי|הסברתי|טענתי|הראיתי|סיכמתי|אסכם|אציין|אדון|אפרט|אטען)(?:\s|,|\.|$)|בפתח\s+הדברים|כפי\s+שציינתי|כפי\s+שהסברתי/;
...
const STRIP_CONNECTIVE_RE = /^(?:עם זאת|לעומת זאת|מנגד|מאידך|בנוסף|כמו כן|יתרה מכך|יתר על כן|זאת ועוד|לפיכך|לכן|על כן|משום כך|כתוצאה מכך|מכאן|כלומר|למעשה)[,\s]+/;
    ...
    switch (move) {
      ...
      case 'explain': {
        ...
        const clause = pickCoreSentence(ev.text, {
          terms, used: usedSentences, rejectAnaphor: !sentences.length,
        });
        if (!clause) return false;
        markUsed(clause);
        const eid = evId(ev); if (eid && !usedEvidenceIds.includes(eid)) usedEvidenceIds.push(eid);
