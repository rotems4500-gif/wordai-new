...
//
// תלויות: LEAF מוחלט — מחרוזות בלבד. הייבוא היחיד הוא styleMarkers.shared, שהוא
// עצמו מודול נתונים באפס תלויות (מערכי מחרוזות + פונקציות טהורות) — כלומר "מחרוזות
// בלבד" נשמר כלשונו, רק שהמחרוזות חיות במקום אחד לכל המערכת.

import { SENTENCE_OPENER_CONNECTIVES } from './styleMarkers.shared.js';
 ...
 * הרשימה **המלאה** מהמקור המשותף — כאן אין תת-קבוצה: הגדרת "מה יכול לפתוח משפט"
 * זהה בשני אתרי-השימוש (כאן ובקילוף של proseComposeService), וההבדל היחיד ביניהם
 * מתועד שם כרשימת-החרגה מפורשת.
 */
const SPLIT_CONNECTIVES = SENTENCE_OPENER_CONNECTIVES;
...
export const COMMA_SLOT_DEFS = [
  ...
  },
  {
    ...
    kind: 'opener',
    // ⚠️ **לא אוחד עם SENTENCE_OPENER_CONNECTIVES בכוונה.** זו אינה רשימת מרקרים
    // אלא הגדרת **משמורת פיסוק נמדדת**: תיאורים שאחריהם בא פסיק. מילות חיבור
    // מתאמות ("אך", "אבל", "אולם", "ברם") יכולות לפתוח משפט אבל **אינן** לוקחות
    // פסיק אחריהן, ולכן הכנסתן לכאן הייתה מזהמת את המדידה של הרגלי המשתמש. מנגד
    // יש כאן מרקרי מניין ("ראשית") שאינם ברשימת הפיצול כלל. שתי הרשימות נבדלות
    // מהותית — איחוד היה מרחיב גם מה שנמדד וגם מה שנאכף.
