...
// ⚠️ הרשימות עצמן חיות ב-styleMarkers.shared (מודול LEAF, מחרוזות בלבד) — כאן אין
// יותר עותק. תוספת מרקר נעשית שם ומגיעה בבת אחת לגלאי, למחולל ולפרומפטים.
// tools/detector-train/extractor.mjs מייבא מאותו קובץ, ולכן אין דרך שהשניים ייפרדו.
import { FORMAL_CONNECTORS, CLICHE_PHRASES, AI_REGISTER_PATTERN } from './styleMarkers.shared';
...
// FORMAL_CONNECTORS (מילות קישור פורמליות) ו-CLICHE_PHRASES (קלישאות שחוקות)
// מיובאים מ-styleMarkers.shared למעלה — הרשימה המלאה, בלי חיתוך. ר' שם את מקורות
// כל קבוצת מרקרים (אימון מקומי / קורפוס מורחב) ואת סיבת הפסיק ב"ראשית,".
...
// ניסוחי מסגור/המלצה "בטוחים" — התבנית עצמה (AI_REGISTER_PATTERN) חיה ב-shared;
// כאן רק נבנה ה-regex עם הדגל g שספירת המופעים דורשת. הבנייה מהמחרוזת שקולה תו-לתו
// לליטרל שהיה כאן קודם.
const AI_REGISTER_RE = new RegExp(AI_REGISTER_PATTERN, 'g');
...
export function extractAuthenticityFeatures(input = '') {
  ...
  // CV-ליבה: אחידות אורך משפטים *מחוץ* לרצפים אנפוריים. אנפורה גוררת מטבעה גם
  // אורכים דומים ("תודה על X" שוב ושוב) — אחרי שהגנו על openerRepeat, המכתב האנפורי
  // עדיין נענש דרך uniformity על אותה תופעה בדיוק. מודדים אחידות רק על המשפטים
  // החופשיים; פחות מ-3 כאלה ⇒ null (אין מספיק מידע להאשים במכונה).
...
function computeSignals(features, profile) {
  ...
  // בטקסט אנפורי (רצף ≥3 של אותו פתיח) האחידות נמדדת על ה-CV-ליבה — המשפטים שמחוץ
  // לרצפים — כי אנפורה גוררת אורכים דומים מעצם טבעה, וכבר קבענו (openerRepeat)
  // שהיא רטוריקה אנושית. בלי זה המכתב האנפורי נענש פעמיים על אותה תופעה.
