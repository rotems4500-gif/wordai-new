...
function extractAnswer(raw) {
  ...
  // ⚠️ היגיינת פורמט, לא איכות. נמדד על DictaLM-3.0-1.7B: «**לטענת יקיר…**»
  // ו-«תשובה: **דליה עשויה…**» — הדגשות מרקדאון ותווית פותחת. הן מנפחות את
  // ספירת המילים ומזהמות את שער האורך, כלומר מודל נפסל על **עיצוב** ולא על
  // תוכן. כל מודל עלול לעשות זאת; הניקוי גנרי ולא תלוי-מודל.
  return pick
    .replace(/^["'״׳\-–—•\s]+/, '')
    .replace(/^(?:תשובה|המשפט|הפלט|תוצאה|משפט)\s*:\s*/, '')
    .replace(/\*\*?([^*]+)\*\*?/g, '$1')
    .replace(/^#+\s*/, '')
    .replace(/\s+/g, ' ')
    .trim();
...
// WORDAI_REWRITE_PROMPT=compact — פרומפט קצר למודל קטן (ר' buildPrompt).
// ---------- מקום אילוץ הנושא בפרומפט ----------
// ⚠️ **הממצא בעל ההשפעה הגדולה ביותר של 27.7, ומחיר אפס.** אילוץ "המשפט חייב
// להזכיר את X" הועבר מראש הפרומפט לשורה שלפני "המשפט:". שום מילה לא שונתה —
// רק המקום:
//
...
//
// כלומר מכריע למודל קטן, וניטרלי-עד-חוסך למודל הייצור (פחות קריאות מודל).
//
// ❌ וההשערה שנשללה בדרך: **לא** אורך הפרומפט. גרסה קומפקטית (בלי דוגמאות
// סגנון ובלי רשימת החוקים) נתנה **2/18** — גרוע מהמקור. דוגמאות הסגנון ורשימת
...
const COMPACT_PROMPT = (() => {
  try { return String(process?.env?.WORDAI_REWRITE_PROMPT || '') === 'compact'; } catch { return false; }
})();
...
function buildPrompt({
  ...
  // ---------- פרומפט קומפקטי למודל קטן ----------
  // ⚠️ נמדד (27.7) על DictaLM-3.0-1.7B: הוא נכשל כמעט תמיד בשער **הנושא** —
  // כותב על הדוקטרינה ולא על הצד שנשאל עליו. וזה לא מפני שלא נאמר לו: הפרומפט
  // המלא אומר זאת פעמיים. זה מפני שההנחיה **נבלעת** — שישה חוקים, דוגמאות
  // סגנון, שלושה בלוקי אזהרה ו-700 תווי מקור לפני נקודת היצירה. מודל 1.7B
  // מאבד אילוץ שנמצא 500 טוקנים מאחור.
  //
  // הגרסה הקומפקטית: אותם אילוצים, בסדר הפוך — המקור והעובדות ראשונים, והאילוץ
  // הקריטי (הצד + האורך) **בשורה האחרונה לפני "המשפט:"**. בלי דוגמאות סגנון
  // (הן מוסיפות ~200 טוקנים ומודל בגודל הזה ממילא לא מחקה מבנה).
  // ❌ **נפסל במדידה (27.7): 4/18 → 2/18 על DictaLM-3.0-1.7B.** ההשערה הייתה
  // שהאילוץ נבלע בהקשר ארוך; הקיצור דווקא הזיק. ההסבר הסביר: דוגמאות הסגנון
  // ורשימת החוקים הן פיגום שמודל קטן **נשען עליו**, לא רעש שמסתיר ממנו.
  // נשאר מאחורי דגל כדי שהמדידה תהיה ניתנת לשחזור.
  if (COMPACT_PROMPT) {
    const who = subjects.length ? subjects.join(' או ') : 'מי שהשאלה עוסקת בו';
    return `המקור:
${String(source).slice(0, 500)}

עובדות המקרה:
${String(caseFacts).slice(0, 700)}

השאלה: ${question}
${previous ? `\nכבר נכתב: "${previous}" — כתוב משהו אחר.\n` : ''}
כתוב משפט אחד בעברית שעונה על השאלה.
המשפט חייב להזכיר את ${who}, להתבסס רק על המקור ועל העובדות, ולא לעלות על ${MAX_SENT_WORDS} מילים.

המשפט:`;
  }

  // WORDAI_REWRITE_PROMPT=subjlast — אותו פרומפט בדיוק, בלוק הנושא מועבר
  // לשורה שלפני היצירה. מפריד **מקום** מ**אורך**: הקיצור נפסל, וזה בודק אם
  // דווקא הקִרבה לנקודת היצירה היא שחסרה.
  const subjLast = SUBJ_LAST && subjects.length;
  const subjHere = subjLast ? '' : subjBlock;
  const subjTail = subjLast
...
המקור:
${String(source).slice(0, 700)}${subjTail}
המשפט:`;
}
